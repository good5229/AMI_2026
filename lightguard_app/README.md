...
LightGuard는 지자체 가로등 자산 데이터와 가명화 공모전 AMI 기반 탐지 결과를 결합해,
점검 우선순위를 제공하는 Flutter MVP입니다. 기존 원격제어시스템을 대체하지 않고
제어상태, 실제 소비전력, 자산 기대부하, 운전시간 context를 비교하는 독립 검증
레이어(Second Checker)를 지향합니다.
...
46/46은 현장 고장탐지 정확도 100%를 의미하지 않습니다. 공모전 AMI 6건도 실제
고장 확정이 아니라 정비 이력 및 현장 확인이 필요한 점검 후보입니다.
